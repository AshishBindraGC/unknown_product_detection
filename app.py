...
output_folder = "/mnt/data/ashish/storage/output/final_data5"
...
def collect_boxes(results, model_id, all_boxes, all_scores, all_labels):
    for box in results.boxes:
        x1, y1, x2, y2 = box.xyxy[0]
        conf = float(box.conf[0])
        cls_id = int(box.cls[0])  # 🔥 no offset

        # Boost score using priority
        boosted_score = conf + MODEL_PRIORITY[model_id]
...
for img_name in os.listdir(input_folder):
    ...
    collect_boxes(r1, 1, all_boxes, all_scores, all_labels)
    collect_boxes(r2, 2, all_boxes, all_scores, all_labels)
    collect_boxes(r3, 3, all_boxes, all_scores, all_labels)
...
print("✅ Priority-based dataset created (no class offset)")
